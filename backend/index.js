  ...
  try {
    ...
    if (process.env.MONGODB_URI && !isConnected) {
      try {
        ...
        console.log(
          "❌ MongoDB connection failed, using file-based storage:",
          error.message
        );
        isConnected = false;
      }
...
function recalculateAnalytics(orders) {
  ...
  const dailySales = Object.keys(ordersByDate).map((date) => {
    ...
    const inProcessOrders = dayOrders.filter(
    ...
    dayOrders.forEach((order) => {
      ...
      shopRevenue[shop] =
        (shopRevenue[shop] || 0) + parseFloat(order.totalAmount || 0);
    });

    const topShop = Object.keys(shopRevenue).reduce(
      (a, b) => (shopRevenue[a] > shopRevenue[b] ? a : b),
      "N/A"
    );
  ...
  const weeklySales = Object.keys(ordersByWeek).map((week) => {
    ...
    const pendingOrders = weekOrders.filter(
      (order) => order.status === "Pending"
    ).length;
    const inProcessOrders = weekOrders.filter(
      (order) => order.status === "In Progress"
    ).length;
  ...
  const predictions = [
    {
      ...
      predictedValue:
        dailySales.length > 0
          ? dailySales[dailySales.length - 1].totalRevenue * 1.1
          : 850.0,
      confidence: 85,
  ...
  const reports = [
    {
      ...
      paymentBreakdown: latestWeekly.paymentBreakdown || {
        Balance: 0,
        Cash: 0,
        Card: 0,
        Bank: 0,
...
async function forceRecalculateAndSaveAnalytics() {
  ...
  const { dailySales, weeklySales, predictions, reports, notifications } =
    recalculateAnalytics(data.orders);
...
// Orders API
app.get('/api/orders', cors(corsOpts),(req, res) => {
  const {
    search,
    status,
    paymentMethod,
  ...
  if (minAmount || maxAmount) {
    filteredOrders = filteredOrders.filter((order) => {
      ...
      if (minAmount && maxAmount) {
        return (
          amount >= parseFloat(minAmount) && amount <= parseFloat(maxAmount)
        );
      } else if (minAmount) {
  ...
  if (startDate || endDate) {
    filteredOrders = filteredOrders.filter((order) => {
      const orderDate = new Date(
        order.deliveredAt || order.createdAt || new Date()
      );
      if (startDate && endDate) {
...
app.put("/api/orders/:id", async (req, res) => {
  try {
    ...
    console.log(
      "💾 Data saved successfully to MongoDB with recalculated analytics"
    );
...
app.post("/api/optimize-route", async (req, res) => {
  try {
    ...
    const activeOrders = ordersToOptimize.filter(
      (order) => order.status === "Pending" || order.status === "In Progress"
    );

    console.log(
      `🔴 Route optimization requested for ${activeOrders.length} active orders`
    );
    ...
    if (startPostcode && MAPBOX_TOKEN) {
      try {
        const depotUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
          startPostcode
        )}.json?access_token=${MAPBOX_TOKEN}&country=GB`;
    ...
    console.log(
      `🔴 Depot coordinates: [${depotCoords.lng}, ${depotCoords.lat}]`
    );
    ...
    for (const order of activeOrders) {
      if (!order.customerPostcode) {
        console.log(
          `⚠️ Order ${order.id} (Basket ${order.basketNo}) skipped - no postcode`
        );
        continue;
      }
      ...
      try {
        console.log(
          `📍 Geocoding: ${order.customerPostcode} for Order #${order.basketNo}`
        );
        ...
        if (response.data.features && response.data.features.length > 0) {
          ...
          console.log(
            `❌ Order #${order.basketNo} - no geocoding results for: ${order.customerPostcode}`
          );
          ordersWithCoordinates.push({
            ...order,
            coordinates: null,
        ...
        console.error(
          `❌ Geocoding error for Order #${order.basketNo}:`,
          err.message
        );
        ordersWithCoordinates.push({
          ...order,
    ...
    console.log(
      `🔴 Valid orders with coordinates: ${validOrders.length}/${activeOrders.length}`
    );
    ...
    while (remaining.length) {
      ...
      console.log(
        `🚗 Route stop ${next.routeOrder}: Order #${next.basketNo} (${
          next.customerPostcode
        }) - ${next.routeDistance.toFixed(2)} km`
      );
    ...
    console.log(
      `🔴 Nearest-neighbor route built with ${optimizedRoute.length} stops`
    );

    // Add orders without coordinates at the end
    const ordersWithoutCoordinates = ordersWithCoordinates.filter(
      (order) => !order.coordinates
    );
    if (ordersWithoutCoordinates.length > 0) {
      console.log(
        `⚠️ Adding ${ordersWithoutCoordinates.length} orders without coordinates at the end`
      );
    ...
    console.log(
      `📋 Final route:`,
      optimizedRoute.map(
        (order) => `#${order.basketNo} (${order.customerPostcode})`
      )
    );
...
app.post("/api/print-route", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="delivery-route.pdf"'
    );
    ...
    doc
      .fontSize(28)
      .font("Helvetica-Bold")
      .text("MAGICSELL", { align: "center" });
    doc
      .fontSize(16)
      .font("Helvetica")
      .text("DELIVERY ROUTE", { align: "center" });
    ...
    doc
      .fontSize(20)
      .font("Helvetica-Bold")
      .text("ROUTE SUMMARY", { align: "center" });
    doc.moveDown(1);
    ...
    doc
      .fontSize(14)
      .font("Helvetica-Bold")
      .text("DELIVERY NOTES:", { underline: true });
    doc.moveDown(0.5);
...
app.get("/api/analytics", (req, res) => {
  try {
    ...
    const { dailySales, weeklySales, predictions, reports } =
      recalculateAnalytics(orders);
    ...
    const todaysOrders = orders.filter((order) => {
      ...
      console.log(
        `📦 Order ${order.basketNo}: createdAt=${
          order.createdAt
        }, orderDate=${orderDate}, isToday=${orderDate === today}`
      );
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        totalRevenue: updatedData.orders.reduce(
          (sum, order) => sum + parseFloat(order.totalAmount || 0),
          0
        ),
      },
...
app.post("/api/calculate-weekly-sales/:week", (req, res) => {
  try {
    ...
    const averageOrderValue =
      weekOrders.length > 0 ? totalRevenue / weekOrders.length : 0;
    ...
    weekOrders.forEach((order) => {
      ...
      paymentBreakdown[method] =
        (paymentBreakdown[method] || 0) + parseFloat(order.totalAmount || 0);
    });
    ...
    weekOrders.forEach((order) => {
      ...
      dailyBreakdown[day] =
        (dailyBreakdown[day] || 0) + parseFloat(order.totalAmount || 0);
    });
...
app.post("/api/predictions/calculate", (req, res) => {
  try {
    ...
    const predictions = calculateAdvancedPredictions(
      historicalData,
      type,
      timeframe
    );
...
const generateInsights = (revenue, orders, average, growthRate) => {
  ...
  if (growthRate > 0) {
    ...
    insights.push({
      ...
      description: `${Math.abs(growthRate).toFixed(
        1
      )}% decrease compared to last period`,
      value: `${growthRate.toFixed(1)}%`,
    });
  }
...
app.post("/api/calculate-daily-sales/:date", (req, res) => {
  try {
    ...
    const averageOrderValue =
      dayOrders.length > 0 ? totalRevenue / dayOrders.length : 0;
    ...
    dayOrders.forEach((order) => {
      ...
      hourlyBreakdown[hour] =
        (hourlyBreakdown[hour] || 0) + parseFloat(order.totalAmount || 0);
    });
