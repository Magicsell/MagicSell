...
function App() {
  ...
  useEffect(() => {
    ...
    if (optimizedRoute.length > 0) {
      console.log(
        "🔄 Route orders:",
        optimizedRoute.map((o) => o.basketNo)
      );
    }
  }, [optimizedRoute]);
  ...
  const [paymentPopover, setPaymentPopover] = useState({
    open: false,
    anchorEl: null,
    paymentMethod: null,
  });
    ...
    newSocket.on("order-updated", (data) => {
      ...
      if (data.deleted) {
        ...
        setFilteredOrders((prevFiltered) =>
          prevFiltered.filter((order) => order.id !== data.orderId)
        );
      } else {
  ...
  const fetchCustomers = async (page = 1, limit = 1000) => {
    try {
      const response = await fetch(
        `${getApiUrl()}/api/customers?page=${page}&limit=${limit}&sortBy=shopName&sortOrder=asc`
      );
      const data = await response.json();
  ...
  const handleOrderDialogOpen = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${getApiUrl()}/api/customers?page=1&limit=1000&sortBy=shopName&sortOrder=asc`
      );
      const data = await response.json();
      ...
      if (data.customers) {
        setCustomers(data.customers);
        console.log(
          "✅ Set customers from paginated response:",
          data.customers.length
        );
        console.log(
          "📋 Customer names:",
          data.customers.map((c) => c.shopName)
        );
      } else {
        // Fallback for old format
        const sortedCustomers = data.sort((a, b) =>
        ...
        setCustomers(sortedCustomers);
        console.log(
          "✅ Set customers from fallback response:",
          sortedCustomers.length
        );
        console.log(
          "📋 Customer names:",
          sortedCustomers.map((c) => c.shopName)
        );
      }
    } catch (error) {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("📊 In Progress Orders:", data.inProcessOrders);
        setAnalytics(data);
      } else {
  ...
  const handleOrderPaymentUpdate = async (orderId, newPaymentMethod) => {
    try {
      console.log("💳 Updating order payment method:", {
        orderId,
        newPaymentMethod,
      });
      ...
      if (response.ok) {
        ...
        if (result.route && result.route.length > 0) {
          const updatedOrders = orders.map((order) => {
            const optimizedOrder = result.route.find(
              (optOrder) => optOrder.id === order.id
            );
          ...
          console.log(
            "🔄 Optimized route state updated, length:",
            result.route.length
          );
          ...
          setNotification({
            open: true,
            message: `Route optimized! Total distance: ${
              result.totalDistance || "N/A"
            } km, Orders: ${result.route.length}`,
            severity: "success",
          });
        } else {
  ...
  const printRoute = async () => {
    try {
      ...
      // Filter orders to only include Pending and In Progress orders
      const activeOrders = orders.filter(
        (order) => order.status === "Pending" || order.status === "In Progress"
      );
  ...
  const handleLogin = (userData) => {
    ...
    setNotification({
      open: true,
      message: `${
        userData.type === "admin" ? "Admin" : "Driver"
            ...
            <Toolbar>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Chip
                  ...
                  sx={{
                    backgroundColor:
                      currentUser?.type === "driver" ? "#FF9800" : "#2196F3",
                    color: "white",
                    fontWeight: "bold",
                  }}
                />
                    ...
                    <Box sx={{ mb: 4 }}>
                      ...
                      <Box sx={{ mb: 3 }}>
                        <Tabs
                          ...
                          onChange={(e, newValue) =>
                            setShowDriverRouteMap(newValue === 0)
                          }
                          centered
                          sx={{ mb: 2 }}
                        >
                      ...
                      {!showDriverRouteMap && (
                        ...
                        >
                          <Typography variant="body2" color="text.secondary">
                            Debug: optimizedRoute.length ={" "}
                            {optimizedRoute.length}, showDriverRouteMap ={" "}
                            {showDriverRouteMap.toString()}
                          </Typography>
                          {optimizedRoute.length === 0 && (
                            <Typography
                              ...
                              sx={{ mt: 1 }}
                            >
                              ⚠️ No optimized route available. Please optimize
                              route first.
                            </Typography>
                          )}
                        </Box>
                  ...
                  {driverView === "orders" && (
                    <Box sx={{ mb: 4 }}>
                      ...
                      <Typography variant="h6" sx={{ mb: 2, color: "#666" }}>
                        📦 Orders to Deliver:{" "}
                        {analytics
                          ? analytics.pendingOrders + analytics.inProcessOrders
                          : orders.filter(
                              (o) =>
                                o.status === "Pending" ||
                            ...
                            {orders
                              ...
                              .sort((a, b) => {
                                const dateA = new Date(
                                  a.createdAt || a.deliveredAt || 0
                                );
                                const dateB = new Date(
                                  b.createdAt || b.deliveredAt || 0
                                );
                                return dateB - dateA;
                              })
                              .map((order) => (
                    ...
                    <Box>
                      {currentUser?.type === "driver" ? (
                        ...
                        <Box>
                          ...
                          {/* Driver için sadece Pending ve In Progress siparişler */}
                          <Box sx={{ mb: 3 }}>
                            <Typography
                              variant="h6"
                              sx={{ mb: 2, color: "#666" }}
                    ...
                    <Box>
                      ...
                      >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            console.log(
                              "🔴 Add Customer button clicked directly"
                            );
                        ...
                        <Table stickyHeader>
                          ...
                          <TableBody>
                            {customers.map((customer) => (
                              <TableRow
                                key={`customer-${customer.id}-${customer.name}`}
                                sx={{ cursor: "pointer" }}
                      ...
                      >
                        <Typography variant="body2" color="text.secondary">
                          Showing{" "}
                          {customerPagination.customersPerPage *
                          ...
                          {Math.min(
                            customerPagination.currentPage *
                              customerPagination.customersPerPage,
                            customerPagination.totalCustomers
                          )}{" "}
                          of {customerPagination.totalCustomers} customers
                        </Typography>
                        <Box sx={{ display: "flex", gap: 1 }}>
                          <Button
                            size="small"
                          ...
                          >
                            Page {customerPagination.currentPage} of{" "}
                  ...
                  {activeTab === 2 && (
                    <Box>
                      ...
                      <Box sx={{ mb: 2, fontSize: "1rem" }}>
                        Interactive map showing optimized delivery route from
                        Poole Depot (BH13 7EX)
                      </Box>

                      {/* <MapComponent 
                orders={orders.filter(order => order.status === 'Pending' || order.status === 'In Progress')}
                optimizedRoute={optimizedRoute}
              ...
              /> */}
                      <MapComponent
                        ...
                        onRouteOptimized={(payload) => {
                          // Hem dizi hem obje ({route:[…]}) gelirse destekle
                          const arr = Array.isArray(payload)
                            ? payload
                            : Array.isArray(payload?.route)
                            ? payload.route
                            : Array.isArray(payload?.optimizedRoute)
                            ? payload.optimizedRoute
                            : [];
                          setOptimizedRoute(arr);
                        }}
                      />
                  ...
                  {activeTab === 4 && (
                    <Box>
                      ...
                      <Box sx={{ mb: 2, fontSize: "1rem" }}>
                        Mobile-optimized interface for drivers to track and
                        deliver orders
                      </Box>
                                  ...
                                  >
                                    {(() => {
                                      ...
                                      const distanceInMiles =
                                        totalDistance * 0.621371; // Convert km to miles
                                  ...
                                  >
                                    {(() => {
                                      ...
                                      const actualDistanceMiles =
                                        actualDistanceKm * 0.621371; // Convert km to miles
                                      const fuelConsumption =
                                        actualDistanceMiles * 0.2; // 0.2L per mile (average of 0.17-0.22)
                                ...
                                >
                                  ...
                                  >
                                    {(() => {
                                      ...
                                      const totalTime = deliveredOrders.reduce(
                                        (sum, order) => {
                                      ...
                                      const avgTime =
                                        deliveredOrders.length > 0
                                          ? totalTime / deliveredOrders.length
                                          : 0;
                            ...
                            <Box sx={{ maxHeight: 300, overflow: "auto" }}>
                              {(() => {
                                ...
                                orders.forEach((order) => {
                                  ...
                                  customerStats[order.customerName].total +=
                                    parseFloat(order.totalAmount || 0);
                                });
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid size={{ xs: 12, md: 6 }}>
                        <Autocomplete
                          ...
                          value={
                            customers.find(
                              (c) => c.shopName === orderForm.shopName
                            ) || null
                          }
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid size={{ xs: 12, md: 6 }}>
                        <FormControl fullWidth sx={{ mb: 2 }}>
                          <InputLabel>
                            💳 Payment Method (Optional - Driver will set)
                          </InputLabel>
                          <Select
          ...
          >
            {paymentPopover.paymentMethod && paymentPopover.orders && (
              <Box sx={{ p: 2 }}>
                ...
                >
                  💳 {paymentPopover.paymentMethod} -{" "}
                  {paymentPopover.orders.length} Orders
                </Box>
                <Divider sx={{ mb: 2 }} />
                <List dense>
