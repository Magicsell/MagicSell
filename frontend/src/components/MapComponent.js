...
const MapComponent = ({ orders, optimizedRoute, onRouteOptimized }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!tokenToUse) {
        setError(
          "Mapbox token gerekli. Lütfen .env dosyasına REACT_APP_MAPBOX_TOKEN ekleyin."
        );
        return;
      }
      ...
      setError(
        "Harita yüklenirken hata oluştu. Lütfen Mapbox token'ını kontrol edin."
      );
  ...
  const addOrderMarkers = async () => {
    ...
    const ordersToDisplay =
      optimizedRoute.length > 0 ? optimizedRoute : filteredOrders;
    ...
    for (let i = 0; i < ordersToDisplay.length; i++) {
      ...
      try {
        ...
        if (knownPostcodes[order.customerPostcode]) {
          ...
          console.log(
            `Using known coordinates for ${order.customerPostcode}:`,
            [lng, lat]
          );
        } else {
          ...
          )}.json?access_token=${tokenToUse}&country=GB&types=postcode&limit=1`;
          ...
          if (data.features && data.features.length > 0) {
            ...
            console.warn(
              `No coordinates found for postcode: ${order.customerPostcode}`
            );
        ...
        if (usedCoordinates.has(coordKey)) {
          console.log(
            `Skipping duplicate coordinates for Order #${order.basketNo} (${order.customerPostcode}): ${coordKey}`
          );
          continue;
        }
        ...
        const routeOrderNumber =
          optimizedRoute.length > 0 ? i + 1 : order.basketNo;
            ...
            new mapboxgl.Popup({
              ...
              offset: 25,
                ...
                <div id="${popupId}" style="min-width: 200px;">
                  <h4 style="margin: 0 0 8px 0; color: #1976d2;">📦 Order #${
                    order.basketNo
                  }</h4>
                  ...
                  <p style="margin: 4px 0;"><strong>📮 Postcode:</strong> ${
                    order.customerPostcode
                  }</p>
                  <p style="margin: 4px 0;"><strong>💰 Amount:</strong> £${
                    order.totalAmount
                  }</p>
                  <p style="margin: 4px 0;"><strong>🚚 Delivery:</strong> ${
                    order.deliveryNo
                  }</p>
              ...
              order.status
                  ...
                  ${
                    order.routeDistance
                      ? `<p style="margin: 4px 0;"><strong>📏 Distance:</strong> ${order.routeDistance.toFixed(
                          2
                        )} km</p>`
                      : ""
                  }
                <p style="margin: 4px 0; font-size: 11px; color: #666;">
                  <strong>📍 Coordinates:</strong> ${lng.toFixed(
                    4
                  )}, ${lat.toFixed(4)}
                </p>
                </div>
        ...
        marker.on("dragend", async () => {
          ...
          try {
            ...
            console.log(
              `Reverse geocoding for new location:`,
              reverseGeocodingUrl
            );
            ...
            if (data.features && data.features.length > 0) {
              ...
              const newPostcode =
                newAddress.context?.find((ctx) => ctx.id.startsWith("postcode"))
                  ?.text || "Unknown";
              ...
              if (popupElement) {
                const addressElement =
                  popupElement.querySelector("p:nth-child(4)"); // Address line
                const postcodeElement =
                  popupElement.querySelector("p:nth-child(5)"); // Postcode line
                const coordinatesElement =
                  popupElement.querySelector("p:last-child"); // Coordinates line
                ...
                if (coordinatesElement) {
                  coordinatesElement.innerHTML = `<strong>📍 Coordinates:</strong> ${newLngLat.lng.toFixed(
                    4
                  )}, ${newLngLat.lat.toFixed(4)}`;
                }
              }
        ...
        marker.getElement().addEventListener("click", () => {
          console.log(
            `Marker clicked for Order ${order.basketNo} (Route Order ${routeOrderNumber}) at ${order.customerPostcode}`
          );
        });
  ...
  const drawRouteLines = async (coordinates) => {
    ...
    try {
      try {
        ...
        const directionsUrl = `https://api.mapbox.com/directions/v5/mapbox/${profile}/${waypoints.join(
          ";"
        )}?geometries=geojson&overview=full&access_token=${tokenToUse}`;
  ...
  const calculateRouteStats = async (coordinates, stopCount) => {
    let totalDistance = 0;
    let totalTime = 0;
    ...
    try {
      ...
      const directionsUrl = `https://api.mapbox.com/directions/v5/mapbox/${profile}/${waypoints.join(
        ";"
      )}?overview=false&access_token=${tokenToUse}`;
    ...
    setRouteStats({
      ...
      totalTime: totalTime,
      totalFuel: totalFuel,
      orderCount: stopCount, // gerçek sipariş sayısı, // Exclude depot
    });
  };
  ...
  const optimizeRoute = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Route optimization failed: ${response.status} - ${errorText}`
        );
      }
      ...
      // }
      const routeArr = Array.isArray(data)
        ? data
      ...
      if (routeArr.length === 0) {
        setError(
          "Optimization API did not return an array; falling back to current order"
        );
      } else if (onRouteOptimized) {
        onRouteOptimized(routeArr);
      }
      ...
      .setPopup(
        new mapboxgl.Popup({
          ...
          order.status
            ...
            <p style="margin: 4px 0; font-size: 11px; color: #666;">
              <strong>📍 Coordinates:</strong> ${lng.toFixed(4)}, ${lat.toFixed(
          4
        )}
            </p>
          </div>
      ...
      try {
        ...
        if (data.features && data.features.length > 0) {
          ...
          if (popupElement) {
            ...
            const postcodeElement =
              popupElement.querySelector("p:nth-child(5)");
            const coordinatesElement =
              popupElement.querySelector("p:last-child");
